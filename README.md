...
Rather than a 'game mode first' experience with the option to launch the desktop (like the Steam Deck), instead it will be a choice of logging onto a full flexible desktop with the option to launch directly into game mode.
...
### Arch Linux installed?
[Gamescope](https://github.com/ValveSoftware/gamescope) is available on the Arch repository and is usually the latest version. Since SteamOS for the Steam Deck is based on Arch Linux it seems the natural choice.
...
### SDDM and KDE Plasma installed?
  - This will make it easier to understanding  the concept and setting up the configurations and scripts
...
### 2. Install MangoHud
- From the command prompt, install the manguhud and lib32-mangohud packages (available under multilib)

  - `sudo pacman -Syu mangohud lib32-mangohud`

### 3. Testing Steam launch in Gamescope
- From the command prompt, run the following command to launch Steam within a Gamescope instance
...
- Run through the set up process by:
  ...
  - Logging in to your Steam account
...
- From the command prompt, run the following command [^1]

  > `gamescope --mangoapp -e -- steam -steamdeck -steamos3`

- This will run Steam again in a Window. Navigate into Settings and you should be able to see the bluetooth settings and other available networks.
...
- Using a code editor, create a new file called `gamescope-session` in the `Developer` folder, add the
following lines to the file and then save the file
...
- Using the code editor, create a new file called `steam.desktop` in the `Developer` folder, add the
following lines to the file and then save the file
...
- From the command line, copy the `steam.desktop` file into `/usr/share/wayland-sessions/` folder
...
- Using a code editor, create a new file named `steamos-session-select` in the `Developer` folder, add the
following lines to the file and then save the file
...
- From the command line, set the permissions to `steamos-session-select` and then copy the script into `/usr/bin/` folder
...
- Test this by either:
  ...
  - Launching in an embeeded session by loggingoff from the desktop (KDE Plasma) and log in again using `Steam (gamescope)` from the session selection
...
### Install MangoHud?
MangoHud is the tool used to allow Steam to display the 'Performance Overlay' for monitoring statistics like frames per second (FPS), resource utilisations, temperatures, etc.

The traditional approach was to set an environment variable `MANGOHUD=1` when launching Steam using Gamescope. MangoHud now comes with MangoApp, which is the preferred application when using Gamescope and MangoHud.
...
Note: This command also enable MangoApp and launch into Steam Deck mode.
...
So although this isn't required for the purpose of this concept, it will be useful to prepare a script to launch Steam using Gamescope and make further tweaks in future.

That said, such tweaks can affect compatibility when running games. Hence keeping things as simple and vanilla as possible seems to be the most compatible way of launching games from Steam.
...
However, with `Switch to Desktop` it tries to invoke a script called `steamos-session-select`. Selecting the `Switch to Desktop` option will not work since the `steamos-session-select` script does not yet exist on our system.
